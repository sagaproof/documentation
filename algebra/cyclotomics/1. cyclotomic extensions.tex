...
% Such an element of $\mu_n$ must exist, otherwise some $X^m-1$ with $m|n$ would have $n>m$ roots. // I don't find this evident
...
% We examine some such field-independent properties in \link[Cyclotomic polynomials]{&cyclotomic polynomials}. // where is that file?
...
\begin{corollary}[Gal(Q(mu_n)/Q) is isomorphic to (Z/nZ)^*]{$\Gal(\Q(\mu_n)/\Q)\cong(\Z/n\Z)^\times$}
...
\begin{theorem}[Gal(F_q(mu_n)/F_q) is isomorphic to <q+Z> of (Z/nZ)^*]{$\Gal(\F_q(\mu_n)/\F_q)$ is isomorphic to $\langle q+\Z\rangle\subseteq(\Z/n\Z)^\times$}
...
\begin{references}
    ...
    \authors{J.S. Milne}
    \when{2021}
    \where{Course Notes - J.S. Milne}
    \other
    \link[Remark 5.11]{www.jmilne.org/math/CourseNotes/FT.pdf#X.5.11}
